...
class DirectAU(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        ...
        if self.encoder_name == 'MF':
            ...
            self.encoder = LGCNEncoder(config,self.n_users, self.n_items, self.embedding_size, self.norm_adj, self.n_layers, self.device)
        else:
...
class LGCNEncoder(nn.Module):
    def __init__(self,config, user_num, item_num, emb_size, norm_adj, n_layers=3, device = "cuda:0"):
