...
class SASGL(GeneralRecommender):
    ...
    def __init__(self, config, dataset):
        super(SASGL, self).__init__(config, dataset)
    ...
    def forward(self, graph):
        ...
        if isinstance(graph, list):
            ...
            for i in range(self.n_layers):
                main_egon = matmul(graph, main_ego)
                score_old, score_new = self.GrowthScore(main_ego, main_egon)
                main_ego = torch.mul(score_old, main_ego) + torch.mul(score_new, main_egon)
                # all_ego.append(main_ego)
