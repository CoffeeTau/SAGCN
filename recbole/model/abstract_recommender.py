...
class GeneralRecommender(AbstractRecommender):
    ...
    The base general recommender class provide the basic datasets and parameters information.
    """
...
class KnowledgeRecommender(AbstractRecommender):
    ...
    The base knowledge-based recommender class provide the basic datasets and parameters information.
    """
    ...
    def __init__(self, config, dataset):
        ...
        if len(self.token_field_dims) > 0:
            self.token_field_offsets = np.array(
                (0, *np.cumsum(self.token_field_dims)[:-1]), dtype=np.float64
            )
        ...
        if len(self.float_field_dims) > 0:
            self.float_field_offsets = np.array(
                (0, *np.cumsum(self.float_field_dims)[:-1]), dtype=np.float64
            )
