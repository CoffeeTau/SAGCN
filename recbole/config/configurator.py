...
class Config(object):
    ...
    def __init__(
        ...
        Args:
            ...
            dataset (str): the datasets name, default is None, if it is None, config will search the parameter 'datasets'
            from the external input as the datasets name.
    ...
    def _init_parameters_category(self):
        ...
        self.parameters["datasets"] = dataset_arguments
    ...
    def _get_model_and_dataset(self, model, dataset):
        ...
        if dataset is None:
            try:
                final_dataset = self.external_config_dict["datasets"]
            except KeyError:
                raise KeyError(
                    "datasets need to be specified in at least one of the these ways: "
                    "[datasets variable, config file, config dict, command line] "
                )
        else:
    ...
    def _load_internal_config_dict(self, model, model_class, dataset):
        ...
        sample_init_file = os.path.join(
            current_path, "../properties/datasets/sample.yaml"
        )
        dataset_init_file = os.path.join(
            current_path, "../properties/datasets/" + dataset + ".yaml"
        )
            ...
            if os.path.isfile(file):
                ...
                if file == dataset_init_file:
                    self.parameters["datasets"] += [
                        ...
                        if key not in self.parameters["datasets"]
                    ]
    ...
    def _set_default_parameters(self):
        self.final_config_dict["datasets"] = self.dataset
    ...
    def _init_device(self):
        ...
        if "local_rank" not in self.final_config_dict:
            ...
            self.final_config_dict["device"] = (
                ...
                else torch.device("cuda:"+self.final_config_dict["gpu_id"])
            )
        else:
