...
class Dataset(torch.utils.data.Dataset):
    """:class:`datasets` stores the original datasets in memory.
    It provides many useful functions for data preprocessing, such as k-core data filtering and missing value
    imputation. Features are stored as :class:`pandas.DataFrame` inside :class:`~recbole.data.datasets.datasets.datasets`.
    General and Context-aware Models can use this class.

    By calling method :meth:`~recbole.data.datasets.datasets.datasets.build`, it will processing datasets into
    ...
    Attributes:
        dataset_name (str): Name of this datasets.

        dataset_path (str): Local file path of this datasets.
    ...
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.dataset_name = config["dataset"] or "gowalla"
        self.logger = getLogger()
        self._from_scratch()

    def _from_scratch(self):
        """Load datasets from scratch.
        Initialize attributes firstly, then load data from atomic files, pre-process the datasets lastly.
        """
    ...
    def _data_filtering(self):
        ...
        Note:
            ...
            thus :meth:`~recbole.data.datasets.datasets.datasets._reset_index` will reset the index of feats.
        """
    ...
    def _build_feat_name_list(self):
        ...
        Any feat loaded by datasets can be found in ``feat_name_list``
    ...
    def _get_download_url(self, url_file, allow_none=False):
        ...
        with open(
            os.path.join(current_path, f"../../properties/datasets/{url_file}.yaml")
        ) as f:
        ...
        if self.dataset_name in dataset2url:
            ...
            raise ValueError(
                ...
                f"nor [{self.dataset_name}] a known datasets name."
            )
    ...
    def _download(self):
        if self.config["local_rank"] == 0:
            ...
            self.logger.info(
                f"Prepare to download datasets [{self.dataset_name}] from [{url}]."
            )
    ...
    def _load_data(self, token, dataset_path):
        ...
        Args:
            token (str): datasets name.
            dataset_path (str): path of datasets dir.
        """
    ...
    def _load_inter_feat(self, token, dataset_path):
        ...
        """
        if self.benchmark_filename_list is None:
            inter_feat_path = os.path.join(dataset_path, "gowalla.inter")
    ...
    def _remove_duplication(self):
        ...
        if self.time_field in self.inter_feat:
            ...
            self.logger.info(
                f"Records in original datasets have been sorted by value of [{self.time_field}] in ascending order."
            )
        else:
            self.logger.warning(
    ...
    def _filter_by_field_value(self):
        ...
        for field, interval in val_intervals.items():
            if field not in self.field2type:
                raise ValueError(f"Field [{field}] not defined in datasets.")
    ...
    @property
    def sparsity(self):
        """Get the sparsity of this datasets.

        Returns:
            float: Sparsity of this datasets.
        """
    ...
    def __str__(self):
        ...
        if self.uid_field and self.iid_field:
            info.append(
                set_color("The sparsity of the datasets", "blue")
                + f": {self.sparsity * 100}%"
            )
    ...
    def copy(self, new_inter_feat):
        """Given a new interaction feature, return a new :class:`datasets` object,
        ...
        Returns:
            :class:`~Dataset`: the new :class:`~datasets` object, whose interaction feature has been updated.
        """
        nxt = copy.copy(self)
    ...
    def split_by_ratio(self, ratios, group_by=None):
        ...
        Returns:
            list: List of :class:`~datasets`, whose interaction features has been split.
    ...
    def build(self):
        """Processing datasets according to evaluation setting, including Group, Order and Split.
        See :class:`~recbole.config.eval_setting.EvalSetting` for details.

        Returns:
            list: List of built :class:`datasets`.
        """
    ...
    def save(self):
        """Saving this :class:`datasets` object to :attr:`config['checkpoint_dir']`."""
        save_dir = self.config["checkpoint_dir"]
        ensure_dir(save_dir)
        file = os.path.join(save_dir, f'{self.config["datasets"]}-datasets.pth')
        self.logger.info(
            set_color("Saving filtered datasets into ", "pink") + f"[{file}]"
        )
    ...
    def inter_matrix(self, form="coo", value_field=None):
        ...
        if not self.uid_field or not self.iid_field:
            raise ValueError(
                "datasets does not exist uid/iid, thus can not converted to sparse matrix."
            )
